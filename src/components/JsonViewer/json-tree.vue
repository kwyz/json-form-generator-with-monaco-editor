<template>
    <span class="json-tree" :class="{'json-tree-root': parsed.depth === 0}">
        <span class="json-tree-row" v-if="parsed.primitive">
            ...
            <span class="json-tree-value" @click="elementTrigger(parsed.value, parsed)" :class="'json-tree-value-' + parsed.type" :title="`${parsed.value}`">{{ `${parsed.value}` }}</span>
        ...
        <span class="json-tree-deep" v-if="!parsed.primitive">
            <span class="json-tree-row json-tree-expando" @click="elementTrigger(parsed.key, parsed)" @mouseover="hovered = true" @mouseout="hovered = false">
                <span class="json-tree-indent">&nbsp;</span>
